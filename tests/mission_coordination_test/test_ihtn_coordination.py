...
def test_mark_elementary_as_concluded():
    ...
    task_state = TaskState(task=lab_samples_ihtn.navto_room.value, status=TaskStatus.SUCCESS_ENDED)
    update_mission(pickup_sample, task_state)
    assert pickup_sample.selected_method.subtasks[0].state.is_status(TaskStatus.SUCCESS_ENDED)
...
def test_mark_all_subtasks_as_concluded():
    ...
    task_status_list = map(lambda t:TaskState(status=TaskStatus.SUCCESS_ENDED, task=t), tasks)
    for ts in task_status_list:
        update_mission(pickup_sample, ts)
    
    # abstract parent task should be SUCCESS_END
    assert pickup_sample.selected_method.subtasks[1].selected_method.subtasks[2].state.is_status(TaskStatus.SUCCESS_ENDED)
