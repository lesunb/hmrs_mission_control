...
def test_create_mission_context(cf_manager: CoalitionFormationProcess, ihtn_collect):
...
def test_flat_plan(cf_manager: CoalitionFormationProcess, ihtn_collect):
    obtained_task = cf_manager.flat_plan(ihtn_collect)
    diff =  set(obtained_task) ^ set([nav_to_room3, pick_up_object])
    assert not diff


def test_get_compatible_workers(cf_manager: CoalitionFormationProcess, ihtn_collect, collection_robots, collection_robots_a_and_b):
    task_list = cf_manager.flat_plan(ihtn_collect,)
    comp_workers = list(cf_manager.get_compatible_workers(task_list, collection_robots))
    # robot_a and robot_b have the skills, robot_c does not
    diff =  set(comp_workers) ^ set(collection_robots_a_and_b)
    assert not diff

def test_check_viable(cf_manager: CoalitionFormationProcess, collection_robots):
    worker = collection_robots[0]
    bid = Bid(worker = worker, estimate = Estimate(time = 3, energy = 5))
    assert cf_manager.check_viable(bid) == True


def test_check_not_viable(cf_manager: CoalitionFormationProcess, collection_robots):
    worker = collection_robots[0]
    bid = Bid(worker = worker, estimate = ImpossibleToEstimate(reason='no route'))
    assert cf_manager.check_viable(bid) == False


def test_sort_and_select_bids(cf_manager: CoalitionFormationProcess, collection_robots):
    worker = collection_robots[0]
